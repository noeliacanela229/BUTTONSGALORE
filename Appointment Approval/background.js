...
async function openReview(message, sender) {
    ...
    if (state === "WA") {
        return await handleWaMvrRouting({
            message,
            vin,
            state,
            rvClass,
...
async function prepareHappyFoxDraft(message, sender) {
    ...
    if (!inquiryId) {
        throw new Error("Unable to create MVR inquiry.");
    }
    ...
    if (yassiRecordStatus !== "success") {
        throw new Error(
            `MVR was not successful: ${describeYassiRecordStatus(yassiRecordStatus)}`
        );
    }
...
async function downloadDebugFromOpenTabs(message, sender) {
    ...
    let vinDecoderScreenshot = null;

    if (!sources?.mvr?.reliabilityComplete) {
        vinDecoderScreenshot =
            await createVinScreenshot(
                vin,
                {
                    active: true,
                    copyToClipboard: true,
...
async function generateReviewOutput(context) {
    ...
    const copyResult =
        vinDecoderScreenshot?.copied
            ? {
                ok: true,
                method: "image/png",
                skippedTextCopy: true
            }
    ...
    return {
        ...
        copied: true,
        vinDecoderOpened,
        mvrStopped: !sources?.mvr?.reliabilityComplete,
        vinDecoderScreenshot,
...
async function handleWaMvrRouting(context) {
    ...
    if (waStorageArea) {
        await waStorageArea.set({
            waPendingVin: context.vin || "",
            waPendingVinSetAt: Date.now()
        });
    }
    ...
    const reviewContext = {
        ...
        yassiTabId: null,
        nmvtisTabId: nmvtisTab?.id || null,
        waTabId: waTab?.id || null
    };

    const nmvtisChecked =
        isMotorized && !!nmvtisId;
...
function buildVinDecoderUrl(vin) {
    ...
    return `https://vpic.nhtsa.dot.gov/decoder/VinDecoder?VIN=${normalizeVin(vin)}&ModelYear=`;
}
